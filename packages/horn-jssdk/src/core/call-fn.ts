...
export function callFn<T>(
  options: AnyOne<Required<Omit<BaseOptions<T>, 'success' | 'fail'>>>,
): Promise<T>
export function callFn<T>(options: any): any {
  ...
  return new Promise<T>((success, fail) => core<T>(Object.assign({}, options, { success, fail })))
}
